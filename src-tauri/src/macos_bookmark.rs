...
fn active_lock() -> std::sync::MutexGuard<'static, Option<HashMap<String, SendUrl>>> {
    // Kilit zehirlenmiş olsa bile içeriği alırız: burada tutulan veri (yol → NSURL) panik
    // sırasında tutarsız kalamaz, bırakılmamış erişim sızdırmak ise gerçek zarardır.
...
pub fn bookmark_release(path: String) -> Result<(), String> {
    let mut guard = active_lock();
    if let Some(SendUrl(url)) = guard.get_or_insert_with(HashMap::new).remove(&path) {
